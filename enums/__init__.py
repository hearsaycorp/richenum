...
class OrderedRichEnum(_EnumMethods):
    """
    Use OrderedRichEnum when you need a RichEnum with index- and slice-based
    access into the enum, e.g. OrderedRichEnumExample[0].

    Values must be OrderedRichEnumValues with indexes defined sequentially,
    with no gaps.

...
from richenum import OrderedRichEnum, OrderedRichEnumValue, RichEnum, RichEnumValue, EnumConstructionException, EnumLookupError
from richenum.enums import _OrderedRichEnumMetaclass
...
class OrderedRichEnumTestSuite(unittest.TestCase):
    ...
    def _test_index(self, zero_indexed):
        ...
        self.assertEqual(self.rich_enum.from_index(self.start_index), self.p1)
        self.assertEqual(self.rich_enum.from_index(self.start_index + 1), self.p2)

        # ensure that non-existent indices raise an exception
        with self.assertRaises(EnumLookupError):
            self.rich_enum.from_index(self.start_index + 2)
